...
import { Image as ImageIcon, Upload, Copy, RefreshCw, X, Check } from "lucide-react";
    ...
    for (const file of Array.from(files)) {
      try {
        ...
        if (res.ok) {
          ...
          setImages(prev => [newUrl, ...prev.filter(i => i !== newUrl)]);
    ...
    <div className="p-4 md:p-6 lg:p-8 max-w-[1400px]" style={{ fontFamily: 'AvantGarde, sans-serif' }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-1">{images.length} media assets loaded</p>
        </div>
        <div className="flex gap-3">
          <button onClick={fetchImages} className="p-2.5 bg-white border border-gray-100 rounded-xl hover:bg-gray-50 shadow-sm flex items-center gap-1 text-xs text-gray-600 font-medium">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-6 gap-3">
          {images.map((url, i) => (
            <div key={i} className={`group relative aspect-square rounded-2xl overflow-hidden bg-[#f1f1f2] border-2 cursor-pointer transition-all ${selected === url ? 'border-[#c81c6a]' : 'border-transparent hover:border-gray-200'}`}
              ...
              <Image src={url} alt={`media-${i}`} fill className="object-cover" sizes="200px" unoptimized={url.startsWith('http')} />
              {/* Overlay */}
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/40 transition-all flex items-center justify-center gap-2 opacity-0 group-hover:opacity-100">
                <button onClick={e => { e.stopPropagation(); copyUrl(url); }}
                  className="w-8 h-8 bg-white rounded-xl flex items-center justify-center hover:bg-[#c81c6a] hover:text-white transition-all shadow-lg text-gray-700">
                  {copied === url ? <Check size={14} /> : <Copy size={14} />}
                </button>
              </div>
              {selected === url && (
                <div className="absolute top-2 right-2 w-5 h-5 bg-[#c81c6a] rounded-full flex items-center justify-center shadow">
      ...
      {selected && (
        <div className="fixed bottom-6 left-1/2 -translate-x-1/2 bg-[#1a1a1a] text-white rounded-2xl px-5 py-3 shadow-2xl flex items-center gap-4 z-40 max-w-[90vw]">
          <div className="w-10 h-10 rounded-xl overflow-hidden flex-shrink-0 relative">
            <Image src={selected} alt="selected" fill className="object-cover" unoptimized={selected.startsWith('http')} />
          </div>
          <p className="text-xs truncate max-w-[200px] text-white/80 font-mono">{selected}</p>
