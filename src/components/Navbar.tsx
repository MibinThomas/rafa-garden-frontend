    ...
    return (
        <nav
            className={`fixed top-0 left-0 right-0 z-40 transition-all duration-500 border-b border-transparent ${scrolled || isMobileMenuOpen
                ...
                : "bg-gradient-to-b from-[#0b2b1a]/80 to-transparent py-4 md:py-8"
                }`}
        >
            <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between">

                {/* Brand/Logo Section - Kerala Theme */}
                <div className="flex items-center gap-2 font-serif text-xl md:text-2xl tracking-wide text-[#fffdd0] drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)] cursor-pointer group z-50">
                    <Leaf className="text-[#38b000] group-hover:text-[#d4af37] transition-colors duration-300" strokeWidth={2.5} size={24} />
                ...
                <div className="flex items-center gap-3 md:gap-4 z-50">
                    {mounted && (
                        <button
                            ...
                            className="p-2 rounded-full border border-[#d4af37]/40 bg-[#123e25]/50 text-[#fffdd0] hover:bg-[#d4af37] hover:text-[#0b2b1a] transition-all shadow-lg backdrop-blur-md"
                    ...
                    <button
                        onClick={openCart}
                        className="relative flex items-center gap-2 px-3 md:px-6 py-2 rounded-full border border-[#d4af37]/40 bg-[#123e25]/50 hover:bg-[#d4af37] hover:border-[#d4af37] transition-all text-[#fffdd0] hover:text-[#0b2b1a] shadow-lg group backdrop-blur-md"
                    >
                        <ShoppingCart size={18} className="group-hover:scale-110 transition-transform" />
                        <span className="hidden md:inline font-semibold tracking-widest text-xs translate-y-[1px] uppercase">Cart</span>
                        {itemCount > 0 && (
