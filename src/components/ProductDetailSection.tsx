...
export function ProductDetailSection({ product, categoryTitle, categoryColor, onClose }: ProductDetailSectionProps) {
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 p-8 lg:p-16">
        {/* Left: Product Image Gallery */}
        <div className="flex flex-row gap-6 h-[500px] lg:h-[600px]">
          {/* Vertical Thumbnails */}
          <div className="flex flex-col gap-4 overflow-y-auto pr-2 custom-scrollbar w-20 md:w-24 shrink-0">
            {images.map((img, idx) => (
              <button 
                key={idx}
                onClick={() => setActiveImageIndex(idx)}
                className={`relative w-20 h-20 md:w-24 md:h-24 rounded-2xl overflow-hidden border-2 transition-all shrink-0 bg-[#f8f8f8] ${activeImageIndex === idx ? 'border-gray-800 shadow-md scale-[1.02]' : 'border-transparent opacity-60 hover:opacity-100'}`}
          ...
          {/* Main Image */}
          <div className="relative flex-1 rounded-[2rem] bg-[#f8f8f8] overflow-hidden group">
            <Image 
           ...
           <div className="absolute top-6 right-6 flex flex-col gap-3">
              <button 
                ...
                className={`w-12 h-12 rounded-full flex items-center justify-center backdrop-blur-md border border-black/5 transition-all shadow-md ${
                  isFavorited ? "bg-red-500 text-white border-transparent" : "bg-white/90 text-gray-400 hover:text-red-500 hover:scale-105"
                }`}
              >
                <Heart size={20} fill={isFavorited ? "currentColor" : "none"} />
              </button>
              <button className="w-12 h-12 rounded-full bg-white/90 backdrop-blur-md border border-black/5 shadow-md flex items-center justify-center text-gray-400 hover:text-[#0b2b1a] hover:scale-105 transition-all">
                <Share2 size={20} />
              </button>
           </div>
          </div>
