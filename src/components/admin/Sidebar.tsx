...
export function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      <div 
        className={cn(
          "fixed inset-0 bg-black/20 backdrop-blur-sm z-[45] lg:hidden transition-opacity duration-500",
      ...
      <aside 
        className={cn(
          "fixed left-0 top-0 h-screen w-72 bg-white/70 backdrop-blur-xl border-r border-white/20 z-50 flex flex-col p-8 shadow-2xl shadow-black/5 transition-transform duration-500 ease-[0.16,1,0.3,1]",
          isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"
        )}
      >
        {/* Brand Header */}
        <div className="mb-14 px-0 flex items-center justify-between">
          ...
          <button 
            onClick={() => setIsOpen(false)}
            className="lg:hidden w-8 h-8 rounded-full flex items-center justify-center bg-gray-100 text-[#5d5f61] active:scale-90 transition-transform"
          >
